...
def binary_search(cmake_parameters: List[str], tools_dir: str) -> Optional[CMakeBinary]:
    ...
    while lower_idx <= upper_idx:
        ...
        remaining_steps = int(
            math.ceil(math.log2(remaining_versions)))  # type: int
        ...
        if result.success:
            ...
            lower_idx = versions.index(proposed_binary[0]) if len(
                proposed_binary) else mid_idx + 1
...
if __name__ == '__main__':
    ...
    if working_version:
        ...
        print('[100%] {message}'.format(message=colored(
            'ERROR: Could not find working version.', 'red')))
